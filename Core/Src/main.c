...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
/*float MQ135_GetRZeroAverage(MQ135* sensor, float t, float h, uint8_t samples) {
    float sum = 0.0f;
    for (uint8_t i = 0; i < samples; i++) {
        float rzero = MQ135_GetCorrectedRZero(sensor, t, h);
        if (rzero > 0) sum += rzero;
        HAL_Delay(100);
    }
    return sum / samples;
}*/
...
int main(void)
{
  ...
  while (1)
  {
    ...
    // Lấy và hiển thị RZero trung bình (100 mẫu)
    /*float rzero_avg = MQ135_GetRZeroAverage(&mq135, 30.0f, 90.0f, 100);
